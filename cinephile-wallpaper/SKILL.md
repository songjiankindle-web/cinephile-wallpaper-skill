...
6. **Create a visual brief**
   ...
   - If the user did not specify an art style, run `scripts/draw-style.mjs` before film research or visual interpretation. Use its returned `style_lane` and `style_variant` exactly. Do not ask the language model to choose, rank, adjust, or justify the style from film content.
   ...
   - Select a concrete `style_lane` only from the script result unless the user specifies a style. Film analysis decides elements, character anchors, props, mood, and metaphor; it must not choose, justify, or override the art style by suitability.
