...
- v1.01 已进入正式发布：迷影桌面核心形态从独立 App 转为跨 agent 的电影壁纸生成 skill/workflow，并强化固定开场、一轮基础设置、影片影调、角色/道具/场景参考图锚点、风格路由、偏好记忆、直接给片名时不得跳过引导流程和低 token 海报优先交付。
...
- 新增 PRD v1.01：`docs/迷影桌面 PRD v1.01.md`，作为当前正式发布 PRD；v0.99a/v0.99b/v1.0 保留为历史发布记录。
...
- `docs/迷影桌面 PRD v1.01.md`：当前正式发布 PRD，覆盖固定开场、一轮基础设置、生图模型路径、统一提示词、影调策略、角色/道具/场景形象参考图、风格路由、偏好记忆、直接给片名时不得跳过引导流程，以及低 token 交付。
