...
- v1.61 已进入正式发布：在 v1.6 全部能力基础上，仅新增 `installation_art` 装置艺术风格路线。
...
- 新增 PRD v1.61：`docs/迷影桌面 PRD v1.61.md`，作为当前正式发布 PRD；v1.6 及更早版本保留为历史发布记录。
...
- `docs/迷影桌面 PRD v1.61.md`：当前正式发布 PRD，在 v1.6 基础上仅增加 `installation_art` 装置艺术风格路线。
- `docs/迷影桌面 PRD v1.6.md`：历史正式发布 PRD，覆盖固定开场、两轮设置、生图模型路径、统一提示词、影调策略、角色/道具/场景形象参考图、风格随机解耦、基础偏好记忆、设计/风格记忆、视觉密度脚本抽签、脚本缓存、分辨率检测失败、图像拒绝降级、直接给片名时不得跳过引导流程、低 token 交付和主 skill 轻量化策略。
...
- 修复第一轮基础设置里的文字版本问题漂移：该问题只能询问“带文字、无文字，还是两者都要”，不得改写为“中英双语”或任何固定语种组合；带文字版语种仍由中文片名 + 影片原产国语种原名规则处理。
- 追加运行层修复：读取偏好时必须把历史保存的“中英双语/双语/bilingual”规范化为 `with_text`/“带文字”，不得继续展示或保存错误标签；LEGO-like + 知名影视/角色组合在生图提示词中必须先去品牌化、去官方标志化，降低图像工具拒绝风险。
- 追加跨对话默认记忆：新增 `scripts/preferences.mjs`，每次新对话在基础设置前读取持久化 JSON；优先使用 `~/.codex/skills/cinephile-wallpaper/preferences.json`，兼容 `~/Documents/电影壁纸/cinephile_preferences.json` 等旧路径。
