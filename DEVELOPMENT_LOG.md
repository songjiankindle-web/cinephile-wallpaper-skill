...
- Skill 新增“角色入镜确认 + 用户上传参考图优先”流程：生图前询问是否希望出现电影角色；若用户选择是，必须请用户上传角色照片/剧照，并把上传图作为本轮最权威参考图。没有可附加参考图时不得生成或承诺可识别人物脸部精准还原。
- 版本归档调整：上一版无强制用户上传参考图闸门的发布候选记为 `v0.99a`；当前带“角色入镜确认 + 用户上传参考图优先”的版本记为 `v0.99b`。
